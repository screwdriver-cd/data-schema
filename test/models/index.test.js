...
describe('model commmons', () => {
    ...
    const modelsToCheck = fs
        .readdirSync(modelsPath)
        .filter(file => fs.statSync(`${modelsPath}/${file}`).isFile() && file !== 'index.js' && /^.*\.js$/.test(file))
        ...
        modelsToCheck.forEach(model => {
            if (Object.prototype.hasOwnProperty.call(models[model], 'rangeKeys')) {
                assert.strictEqual(
                    ...
                    `${model} model: Each range key must matches up with an element in the indexes property.`
                );
            }
        });
    });
                    ...
                    models[model].base.$_terms.keys.forEach(column => {
                        ...
                        if (column.key === columnName && schema.type === 'string') {
                            const result = !schema.rules
                                ? undefined
                                : schema.rules.find(rule => rule.name === 'max' || rule.name === 'length');

                            assert.isDefined(
                                result,
                                `${model}.${columnName} schema must have (valids|length|max) definition.`
                            );
                        }
                    });
                });
